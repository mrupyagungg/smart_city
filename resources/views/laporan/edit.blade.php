            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('pengeluaran.update', ['pengeluaran' => $pengeluaran->id]) }}" method="post">
                        ...
                        <fieldset disabled>
                            <div class="mb-3">
                                <label for="kode_pengeluaran">ID</label>
                                <input class="form-control form-control-solid" id="kode_pengeluaran" name="kode_pengeluaran"
                                    type="text" placeholder="Contoh: S-1" value="{{ $pengeluaran->kode_pengeluaran }}"
                                    readonly>
                            </div>
                        </fieldset>

                        <input type="hidden" id="kode_pengeluaran" name="kode_pengeluaran"
                        ...
                        <div class="mb-3">
                            ...
                            <select id="akun_id" class="form-control" disabled>
                                ...
                                @foreach ($akun as $a)
                                    <option value="{{ $a->id }}"
                                        {{ $pengeluaran->akun_id == $a->id ? 'selected' : '' }}>
                                        {{ $a->nama_akun }}
                                    </option>
                                @endforeach
                            </select>
                            <!-- Input hidden untuk tetap mengirim akun_id -->
                            <input type="hidden" name="akun_id" value="{{ $pengeluaran->akun_id }}">
                        </div>

                        <div class="mb-3">
                            <label for="deskripsi" class="form-label">Deskripsi</label>
                            <textarea class="form-control" id="deskripsi" name="deskripsi" required>{{ old('deskripsi', $pengeluaran->deskripsi) }}</textarea>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" id="jumlah" name="jumlah"
                                    value="{{ number_format(old('jumlah', $pengeluaran->jumlah), 0, ',', '.') }}" required
                        ...
                        <!-- Tombol Batal -->
                        <a class="col-sm-1 btn btn-danger btn-sm" href="{{ url('/pengeluaran') }}"
                            role="button">Batal</a>
                    </form>
